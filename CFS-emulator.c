...
#define wait_time 1200 //1200 ms a process wait for I/O operation (or event)
#define sleep_time 1000 //1 second artificial sleep time to make output readable
...
//****************** CONSTANTS ************************************************
const char* sched_to_string[] = { //string representation of scheduling schemes
	"SCHED_RR",
	"SCHED_FIFO",
	"SCHED_NORMAL"
};
//****************** GLOBAL VARIABLES *****************************************
int p_count[NUM_CPU];//stores number of processes each CPU has
...
void static execute_FIFO(int CPU, struct process_t *process);
void static execute_RR(int CPU, struct process_t *process);
void static execute_NORMAL(int CPU, struct process_t *process);
/** balancer thread **/
...
int main(int argc, char const *argv[])
{
	...
	res = pthread_create(&producer_thread, NULL, producer, (void *)NULL);
    if (res != 0) 
    {
	...
	for (i=0; i<NUM_CPU; i++)
	{
    	...
    	res = pthread_create(&consumer_thread[i], \
    							&thread_attr, consumer, (void *)&cpu_count[i]);
    	if (res != 0) 
    	{
...
//
//! \Internal
//! The function convert a process information string into a process struct
//!
...
struct process_t ltop(char *line)
{
	...
	if (strcmp(token,sched_to_string[RR]) == 0)
		process.sched = RR; //Round Robin sched
	if (strcmp(token,sched_to_string[FIFO]) == 0)
		process.sched = FIFO; //FIFO sched
	if (strcmp(token,sched_to_string[NORMAL]) == 0)
		process.sched = NORMAL; // normal sched
		...
		for (j = 0; j<NUM_QUEUE; j++)
		{
			...
			for (k = queue.out; k<queue.in; k++)	
			{
				...
				printf("%-3d | %-12s | %-3d | %-8d | %-11d | %-4d | %-9d \n",\
						process.pid, sched_to_string[process.sched], i, process.last_cpu,\
	...
	{	
		for (i = 0; i<NUM_QUEUE; i++) // go thru each queue
		{
			while (running) //continuously executes processes in the queue
			{
				...
				printf("[CPU %d]: process PID %d is selected,%s, priority = %d.\n",\
				...
				switch(process->sched)
				{
					...
					default:
						/* Invalid process */
						printf("[CPU %d]: PROCESS PID %d has invalid scheduling scheme.\n",\
						 		CPU, process->pid);
				}
			}
		}
...
//
//! \Internal
//! This method get called by the CPU to execute a SCHED_FIFO process
//!
//! \param pointer to struct process
//!
//! \return NONE
//
//*****************************************************************************
...
//
//! \Internal
//! This method get called by the CPU to execute a SCHED_RR process
//!
//! \param pointer to struct process
//!
//! \return NONE
...
void static execute_RR(int CPU, struct process_t *process)
{
	...
	if (remaining_exec_time > process->time_slice) 
	{
		...
		printf("[CPU %d]: process PID %d is preempted. Remaining execution time = %d.\n"\
...
//
//! \Internal
//! This method get called by the CPU to execute a SCHED_NORMAL process
//!
//! \param pointer to struct process
//!
//! \return NONE
...
void static execute_NORMAL(int CPU, struct process_t *process)
{
	...
	process->sleep_avg = (process->sleep_avg < MAX_SLEEP_AVG)?				\
							process->sleep_avg:MAX_SLEEP_AVG;
	...
	printf("[CPU %d]: executing process PID %d, %s, service_time = %d.\n", \
			CPU, process->pid, sched_to_string[process->sched], service_time);
	if (remaining_exec_time > service_time) 
	{
		...
		if (service_time < process->time_slice)
		{ //if the process is waiting for I/O event
			printf("[CPU %d]: process PID %d is blocked waiting for an event.\n",\
					CPU, process->pid);
		...
		if (process->prio < 130)
		{ // if prio < 130, process go to queue 1
			pthread_mutex_lock(&QM[CPU][1]);
			append(process, &RQ[CPU][1]);
			pthread_mutex_unlock(&QM[CPU][1]);
		}
...
void *balancer(void *arg)
{
	...
	while (running)
	{
		...
		if (n_process <= 0) 
		{
			printf("[Balancer] No more process to execute. The emulator stop.\n");
			running = 0;
			break;
		}
		/* update the b_value */
		...
		** processes are distributed equally to all CPUs */
		...
		for (i = 0; i < NUM_CPU; i++)
		{
			...
			//while CPU has more than b_value processes
			{
				...
				// update process info
				process->last_cpu = i;
				printf("[Balancer]: process PID %d is moved from CPU %d to CPU %d.\n",\
						process->pid, i,j);
